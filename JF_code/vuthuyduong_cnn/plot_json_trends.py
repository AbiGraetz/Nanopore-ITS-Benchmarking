...
def load_json_files(folder: pathlib.Path, sub_str1:str, sub_str2:str) -> List[pathlib.Path]:
    ...
    for fp in folder.iterdir():
        if fp.suffix.lower() == ".json" and sub_str2 in fp.name:
            json_files.append(fp)
    if not json_files:
        sys.exit("can not find json files")
...
def extract_max_values(file_path: pathlib.Path) -> Dict[str, float]:
    ...
    if not isinstance(data, dict):
        sys.exit(f"{file_path} not a dict")
...
def main():
    ...
    parser.add_argument("json_folder", type=pathlib.Path,
                        help="folder that contains json files")
    ...
    if not args.json_folder.is_dir():
        sys.exit(f"not a folder：{args.json_folder}")
    ...
    if len(keys) != 3:
        sys.exit(f"JSON should has 3 keys, but got {len(keys)}：{keys}")
