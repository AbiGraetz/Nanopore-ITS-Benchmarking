...
# cite from https://github.com/vuthuyduong/fungiclassifiers/blob/master/models/CNN/trainCNN.py
...
# 				if len(classification) > classdict[classname]['classification']:
	...
	print()
	print(newfastafilename)
	print(len(newseqrecords))
	print()
...
def load_matrix(matrixfilename,seqids,classnames,classdict):
	...
	X=[]
	Y=[]
	classdict_keys = list(classdict.keys())
	f0 = open("species_list.txt", "w")
	for i in range(len(classdict_keys)):
		f0.write(classdict_keys[i])
		f0.write("\t")
	f0.close()
	for vector in vectors:
		...
		index=classdict_keys.index(taxonname)
		Y.append(index)
		X.append(elements[1:])
...
# 	#load data
	...
	command="python " + path + "fasta2matrix.py " +  str(k) + " " + newfastafilename + " " + matrixfilename
	print(command)
...
# 	print(var)
	...
	print("trainning")
	for i in range(20):
		model.fit(traindata, trainlabels_bin, nb_epoch=5, batch_size=20, verbose = 0)
		model.save(out_path + "test_species_all_" + str(i) + ".keras")
# 	#save model
	modelname=filename.replace(".","_") + "_cnn_classifier"
# 	#save model
	classifiername=modelname + "/" + basename + ".classifier"
	model.save(out_path + "model.keras")
