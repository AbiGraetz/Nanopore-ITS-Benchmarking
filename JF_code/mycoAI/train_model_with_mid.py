...
# cite from https://github.com/MycoAI/MycoAI/blob/master/mycoai/scripts/train.py
...
def train(fasta_filepath, output_filepath=utils.OUTPUT_DIR + 'model.pt', 
    ...
    for i in range(10):
        model, history = SeqClassTrainer.train(
            model, train_data, valid_data, epochs=5, loss=loss, 
            batch_size=batch_size, optimizer=optimizer, levels=levels, 
            label_smoothing=ls
        )
        torch.save(model, output_filepath[:-3] + "_" + str(i) + ".pt")
