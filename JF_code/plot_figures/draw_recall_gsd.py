...
f_out_data = open(out_path + rank_level + "_level_" + dataset_index[0] + "_" + dataset_index[1] + "_" + ref_set + "_recall.csv", 'w')
f_out_data_rate = open(out_path + rank_level + "_level_" + dataset_index[0] + "_" + dataset_index[1] + "_" + ref_set + "_recall_rate.csv", 'w')
...
for i in range(len(file_list)):
    ...
    if result_paths[i].split('/')[6] == "emu_mix":
        ...
        if rank_level == "genus":
            correct_classified_genus = np.array(list(df_sorted['correctly mapped'])[:54])
            correct_classified = np.array(list(df_sorted['correctly mapped'])[:54])
        if rank_level == "phylum":
            correct_classified_phylum = np.array(list(df_sorted['correctly mapped'])[:54])
            correct_classified = np.array(list(df_sorted['correctly mapped'])[:54])
        print(correct_classified_genus)
        ...
        for key in common_species:
            ...
            for key2 in classify_result:
                if rank_level == "genus":
                    if (key2.split(',')[-2] == species_to_genus[key] and not key2.split(',')[-2][-1] == '_') or\
                    (key2.split(',')[-2][:-1] == species_to_genus[key] and key2.split(',')[-2][-1] == '_'):
                        correct_count += classify_result[key2]
                if rank_level == "phylum":
                    if len(key2.split(',')) > 1 and key2.split(',')[1] == species_to_phylum[key]:
                        correct_count += classify_result[key2]
                if rank_level == "species":
                    if key2.split(',')[-1] == key:
                        correct_count += classify_result[key2]
    ...
    f_out_data.write(",".join(map(str, np.array(correct_classified))))
    f_out_data_rate.write(",".join(map(str, (np.array(correct_classified) / mock_base).tolist())))
    ...
    plt.scatter(range(i * 65 + 1, i * 65 + len(common_species) + 1), np.array(correct_classified) / mock_base, \
             color=colors[i], s = 15)
             
    plt.scatter(np.mean(np.array(range(i * 65 + 1, i * 65 + len(common_species) + 1))), np.mean(np.array(correct_classified) / mock_base), marker='s', \
...
plt.legend(loc='best')
save_filepath = out_path + rank_level + "_level_" + dataset_index[0] + "_" + dataset_index[1] + "_" + ref_set + "_recall.png"
